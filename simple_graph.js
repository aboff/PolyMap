...
Drawing.SimpleGraph = function(options) {
  ...
  function createGraph() {
    ...
    $.getJSON( "api/data.json", function( data ) {
      var items = [];
      var dataCount = 1;
      $.each( data, function( key, newNode ) {
        var target_node = new GRAPHVIS.Node(newNode.ID);
        if(graph.addNode(target_node)) {
          target_node.data.title = newNode.data.title;//"This is node " + target_node.id;
          drawNode(target_node);
          nodes.push(target_node);
          $.each(newNode.nodesFrom, function(key2, nodeNumber) {
            for(var i=0; i < nodes.length; i++) {
              node = nodes.filter(function (n) { return n.id == nodeNumber}).shift();
              if(node != null && graph.addEdge(node, target_node)) {
                 //console.log("drawing node " + node.data.title + " + " + target_node.data.title);
                 drawEdge(node, target_node);
              }
            }
          });
          $.each(newNode.nodesTo, function(key2, nodeNumber) {
            for(var i=0; i < nodes.length; i++) {
              node = nodes.filter(function (n) { return n.id == nodeNumber}).shift();
              if(node != null && graph.addEdge(node, target_node)) {
                 //console.log("drawing node " + node.data.title + " + " + target_node.data.title);
